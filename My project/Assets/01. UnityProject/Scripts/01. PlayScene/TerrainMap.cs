...
public class TerrainMap : TileMapController
{
    ...
    public override void InitAwake(MapBoard mapController_)
    {
        ...
        // { X 축 상의 두 타일과, Y 축 상의 두 타일 사이의 로컬 포지션으로 타일 갮을 연산한다.
    ...
    private void Start()
    {
        ...
        GameObject changeTilePrefap = ResManager.Instance.terrainPrefabs[RDefine.TERRAIN_PREF_OCEAN];
        ...
        for (int i = 0; i < allTileObjs.Count; i++)
        {
            ...
            tempChangeTile = Instantiate(changeTilePrefap, tileMap.transform);
            tempChangeTile.name = changeTilePrefap.name;
    ...
    //! 초기화된 타일의 정보로 연산할 맵의 가로, 세로 크기를 리턴한다.
    public Vector2Int GetCellSize() { return mapCellSize; }
    //! 초기화된 타일의 정보로 연산할 타일 사이의 갭을 리턴한다.
